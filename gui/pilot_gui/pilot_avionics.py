...
class PilotAvionics(QMainWindow):
    ...
    def connect_signals(self):
        ...
        if self.btn_toggle:
            self.btn_toggle.clicked.connect(self.toggle_structured_query)
    ...
    def toggle_structured_query(self):
        """구조화된 질의 시스템 토글"""
        if not self.controller:
            return
        
        current_state = self.controller.use_structured_query
        new_state = not current_state
        
        self.controller.toggle_structured_query(new_state)
        
        if self.btn_toggle:
            self.btn_toggle.setText("TOGGLE SQ" if new_state else "LEGACY")
        if hasattr(self, 'statusbar') and self.statusbar:
            self.statusbar.showMessage(f"Structured Query: {'Enabled' if new_state else 'Disabled'}")
