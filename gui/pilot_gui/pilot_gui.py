...
class PilotAvionics(QMainWindow):
    ...
    def load_ui(self):
        ...
        self.status_structured_query = self.findChild(QLabel, "status_structured_query")
        ...
        if self.text_stt_result:
            ...
            print(f"[GUI] FAIL STT 위젯이 None입니다!")
        ...
        if self.text_tts_response:
            ...
            print(f"[GUI] FAIL TTS 위젯이 None입니다!")
    ...
    def setup_event_handlers(self):
        ...
        try:
            ...
            # 이벤트 매니저 초기화
            self.event_manager = EventManager(server_host="localhost", server_port=5300)
            ...
            self.event_manager.register_handler("RUNWAY_ALPHA_STATUS_CHANGED", self.on_runway_alpha_changed)
            self.event_manager.register_handler("RUNWAY_BRAVO_STATUS_CHANGED", self.on_runway_bravo_changed)
    ...
    def on_bird_risk_changed(self, event_data: dict):
        ...
        if self.event_processor:
            ...
            QTimer.singleShot(0, lambda: self.update_bird_risk_display(result))
            
            # 새로운 EventTTS 사용
            if self.event_tts:
                QTimer.singleShot(100, lambda: self.event_tts.play_event_notification(event_type, processed_event.get("result", "UNKNOWN")))
        else:
            # 폴백: 기존 방식
            result = event_data.get("result", "UNKNOWN")
            QTimer.singleShot(0, lambda: self.update_bird_risk_display(result))
            QTimer.singleShot(100, lambda: self.play_event_tts_notification(result, "bird_risk"))
    ...
    def on_runway_alpha_changed(self, event_data: dict):
        ...
        if self.event_processor:
            ...
            # GUI 업데이트 (메인 스레드에서 실행)
            QTimer.singleShot(0, lambda: self.update_runway_alpha_display(result))
            ...
            result = event_data.get("result", "UNKNOWN")
            QTimer.singleShot(0, lambda: self.update_runway_alpha_display(result))
            QTimer.singleShot(100, lambda: self.play_event_tts_notification(result, "runway_alpha"))
    ...
    def on_runway_bravo_changed(self, event_data: dict):
        ...
        if self.event_processor:
            ...
            # GUI 업데이트 (메인 스레드에서 실행)
            QTimer.singleShot(0, lambda: self.update_runway_bravo_display(result))
            ...
            result = event_data.get("result", "UNKNOWN")
            QTimer.singleShot(0, lambda: self.update_runway_bravo_display(result))
            QTimer.singleShot(100, lambda: self.play_event_tts_notification(result, "runway_bravo"))
    ...
    def update_bird_risk_display(self, risk_level: str):
        """조류 위험도 디스플레이 업데이트"""
        if hasattr(self, 'label_bird_risk') and self.label_bird_risk:
            ...
            self.label_bird_risk.setText(f"BIRD RISK: {display_text}")
            
            # 색상 설정
            if risk_level == "BR_HIGH":
                self.label_bird_risk.setStyleSheet("color: #ff4444; font-weight: bold;")
            elif risk_level == "BR_MEDIUM":
                self.label_bird_risk.setStyleSheet("color: #ffaa00; font-weight: bold;")
            else:
                self.label_bird_risk.setStyleSheet("color: #00ff00; font-weight: bold;")
    
    def update_runway_alpha_display(self, status: str):
        """활주로 알파 상태 디스플레이 업데이트"""
        if hasattr(self, 'label_runway_alpha') and self.label_runway_alpha:
            # TCP 결과를 GUI 표시용으로 변환
            display_mapping = {
                "RWY_A_CLEAR": "CLEAR",
            ...
            self.label_runway_alpha.setText(f"RWY ALPHA: {display_text}")
            
            # 색상 설정
            if status == "RWY_A_CLEAR":
                self.label_runway_alpha.setStyleSheet("color: #00ff00; font-weight: bold;")
            else:
                self.label_runway_alpha.setStyleSheet("color: #ff4444; font-weight: bold;")
    
    def update_runway_bravo_display(self, status: str):
        """활주로 브라보 상태 디스플레이 업데이트"""
        if hasattr(self, 'label_runway_bravo') and self.label_runway_bravo:
            # TCP 결과를 GUI 표시용으로 변환
            display_mapping = {
                "RWY_B_CLEAR": "CLEAR",
            ...
            self.label_runway_bravo.setText(f"RWY BRAVO: {display_text}")
            
            # 색상 설정
            if status == "RWY_B_CLEAR":
                self.label_runway_bravo.setStyleSheet("color: #00ff00; font-weight: bold;")
            else:
                self.label_runway_bravo.setStyleSheet("color: #ff4444; font-weight: bold;")
        ...
        try:
            ...
            while self.mic_monitoring_active:
                try:
                    ...
                    # 🔧 MIC LEVEL 민감도 조정 (덜 민감하게)
                    NOISE_THRESHOLD = 300  # 노이즈 임계값 높임 (100→300)
                    if rms > NOISE_THRESHOLD:
                        # 임계값 이상의 신호만 처리 (더 큰 분모로 덜 민감하게)
                        normalized_level = min(100, int((rms - NOISE_THRESHOLD) / 20))
                        # 증폭 계수 감소 (2→1.5)
                        normalized_level = min(100, int(normalized_level * 1.5))
                    else:
    ...
    def update_system_status_display(self):
        ...
        status_labels = {
            ...
            'structured_query_enabled': self.status_structured_query,
