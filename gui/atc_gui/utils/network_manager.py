...
class NetworkManager(QObject):
    ...
    def _connect_signals(self):
        ...
        # UDP 연결 상태 시그널 연결
        self.udp_client.connected.connect(self._on_udp_connected)
        self.udp_client.disconnected.connect(self._on_udp_disconnected)
        self.udp_client.connection_error.connect(self._on_udp_connection_error)

    def start_services(self):
        """네트워크 서비스 시작"""
        logger.info("네트워크 서비스 시작")
        self.udp_client.connect()
        # GUI가 준비될 시간을 번 후 TCP 연결 시도
        QTimer.singleShot(1000, self.tcp_client.connect_to_server)
    ...
    def _on_udp_connected(self):
        """UDP 연결 성공 시 상태 전파"""
        self.udp_connection_status_changed.emit(True, "UDP 서버에 연결되었습니다.")

    def _on_udp_disconnected(self):
        """UDP 연결 해제 시 상태 전파"""
        self.udp_connection_status_changed.emit(False, "UDP 서버 연결이 해제되었습니다.")
