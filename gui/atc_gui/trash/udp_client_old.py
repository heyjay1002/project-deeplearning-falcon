...
class UdpClient(QObject):
    ...
    def connect(self, host, port):
        ...
        try:
            ...
            logger.info(f"UDP 연결 시도: {self.settings.server.udp_ip}:{self.settings.server.udp_port}")
            self.connection_status_changed.emit(True, f"서버 {host}:{port} 연결됨")
            logger.info(f"서버 {host}:{port} 연결 성공")
            return True
        except Exception as e:
            logger.error(f"서버 {host}:{port} 연결 실패: {str(e)}")
            self.is_connected = False
            self.connection_status_changed.emit(False, f"서버 {host}:{port} 연결 실패")
            return False 
