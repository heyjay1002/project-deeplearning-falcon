    ...
    def get_content(self) -> str:
        ...
        if self.notification_type == 'object':
            return f"객체 ID: {self.data.object_id}\n" \
                   f"종류: {self.data.object_type.value}\n" \
                   f"위치: {self.data.zone.value}"
        elif self.notification_type == 'bird':
            return f"조류 위험도: {self.data.value if hasattr(self.data, 'value') else self.data.value}"
        elif self.notification_type == 'runway_a_risk':
            return f"활주로 A 위험도: {self.data.value if hasattr(self.data, 'value') else self.data.value}"
        elif self.notification_type == 'runway_b_risk':
            return f"활주로 B 위험도: {self.data.value if hasattr(self.data, 'value') else self.data.value}"
